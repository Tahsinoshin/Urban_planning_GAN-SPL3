...
def encoder_model(input_dim,latent_dim):
    ...
    out_e = tf.keras.layers.Dense(latent_dim)(h_e)
...
def compute_loss(model, x, func_zone, cond_dim):
    ...
    x_rec = model.decoder(z_c)
    x_origin = x[:, :-cond_dim]
    rec_loss = tf.keras.losses.MeanSquaredError(reduction=tf.keras.losses.Reduction.NONE)(x_origin,x_rec)
...
np.savez_compressed("./tmp/cluvae^_generate_result4.npz",result)
