...
def make_generator_model():
    ...
    model.add(layers.Dense(5*5*256, use_bias=False, input_shape=(15,)))
...
for epoch in range(1, 50 + 1):
  ...
  for batch_ind, data in enumerate(train_dataset):
      batch_urban = tf.reshape(data[0], (-1, 25, 25, 20))
      batch_env = data[2]
...
generate_us = generate_us.reshape((generate_us.shape[0], 25, 25, 20))
np.savez_compressed("../tmp/25_cgan_generate_result.npz", generate_us)
