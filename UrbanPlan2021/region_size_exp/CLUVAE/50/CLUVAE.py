...
def read_data():
    ...
    urban_sol = np.load("../../../data/50_poi_dis.npz")["arr_0"]
    func_zone = np.load("../../../data/func1_50.npz")["arr_0"]
...
def get_params():
    ...
    parser.add_argument("--input_dim", type=int, default=50015, help="the dimension of conditional vector.")
...
def compute_loss(model, x, func_zone, cond_dim):
  ...
  return -tf.reduce_mean(logpx_z + logpz - logqz_x) + 0.55 * func_loss
...
result = generate_us.reshape((generate_us.shape[0],50,50,20))
np.savez_compressed("../tmp/50_cluvae_generate_result.npz",result)
