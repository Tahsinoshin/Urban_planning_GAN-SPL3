...
def gradient_penalty(generator, good_plan, generated_plan):
    ...
    alpha = tf.random.uniform([batch_size, 50, 50, 20], 0., 1.)
...
for epoch in range(1, 50 + 1):
  ...
  for batch_ind, data in enumerate(train_dataset):
      batch_urban = tf.reshape(data[0], (-1, 50, 50, 20))
      batch_env = data[2][:, :-5]
...
generate_us = generate_us.reshape((generate_us.shape[0], 50, 50, 20))
np.savez_compressed("../tmp/50_wgangp_generate_result.npz", generate_us)
